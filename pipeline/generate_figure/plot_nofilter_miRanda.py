...
# all score
tmp_df = data.groupby(by=['mir_score'])['nor_readcount','nor_count'].sum().reset_index()
for rc_type in ['nor_readcount', 'nor_count']:
    fig, ax = plt.subplots(1,1, figsize=(6, 4), dpi=200)
    ax.plot(tmp_df['mir_score'], tmp_df[rc_type], color='firebrick')
    ax.set_xlabel('miRanda score')
    ax.set_ylabel('Read counts')
    plt.savefig('figure/{}_{}_miRanda_score.{}'.format(d_name,rc_type, fig_type), bbox_inches='tight')
    plt.clf()
    plt.close()
    gc.collect()
...
#             ax2.axvline(x=21, c='k', linestyle='dashed', linewidth=0.5)
            ...
            if sd == 1:
                ...
                sd_type = 'one_third'
            plt.savefig('figure/distribution_plot/{}_{}_{}_{}_miRanda.{}'.format(d_name,mut,sd_type,rc_type, fig_type))
            plt.clf()
            plt.close()
            gc.collect()
            ...
            for rc_type in ['nor_readcount', 'nor_count']:
                ...
                for i in range(1,reg_seq_len+1): 
                    ...
                    if sd == 1:
                        ...
                        sd_type = 'one_third'
                    if plot_diff == False:
                        plt.savefig('figure/pairing_ratio_plot/plot21/{}_{}_{}_{}_{}_miRanda.{}'.format(str(i),d_name,mut,sd_type,rc_type, fig_type), bbox_inches='tight')
                    plt.clf()
                    plt.close()
                    gc.collect()
                ...
                if sd == 1:
                    ...
                    sd_type = 'one_third'
                if plot_diff == False:
                    plt.savefig('figure/pairing_ratio_plot/plot_all/{}_{}_{}_{}_miRanda.{}'.format(d_name,mut,sd_type,rc_type, fig_type))
                plt.clf()
                plt.close()
                gc.collect()
                ...
                for box in ['seed', 'non-seed']:
                    for diff in [True, False]:
                        pos = 23 # maximum x-axis position
                        new_df = pd.DataFrame()
                        for i in range(pos+1):
                            ...
                            tmp_df['y'] = y
                            tmp_df['region'] = ['non-seed']+['seed']*7+['non-seed']*(reg_seq_len-8)
                        ...
                        if diff:
                            ...
                            ax.set_title(box)
                            if plot_diff:
                                plt.savefig('figure/pairing_ratio_plot/plot_diff/{}_{}_{}_{}_{}_diff_miRanda.{}'.format(d_name, mut, sd_type, rc_type, box, fig_type), bbox_inches='tight')
                        else:
                            ## 20240918 add horizontal line to show median of leftmost bar ##
                            med = np.median(plot_df[plot_df['x'] == 0]['y'].to_numpy())
                            print(plot_df[plot_df['x'] == 0]['y'].to_numpy())
