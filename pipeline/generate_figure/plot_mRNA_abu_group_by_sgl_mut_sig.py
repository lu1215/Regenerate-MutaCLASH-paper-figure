...
for group in [0]:
    ana_data = add_two_mRNA_list(data, target, group).copy()

    # === 新的分組鍵（轉成布林並補缺） ===
    cD = 'single_read_mut_stat_significance_D'
    cM = 'single_read_mut_stat_significance_M'
    for c in (cD, cM):
        ana_data[c].fillna(False).astype(bool)
        # ana_data[c] = ana_data[c].fillna(False)
        # if ana_data[c].dtype != bool:
        #     ana_data[c] = (ana_data[c].astype(str)
        #                                 .str.strip()
        #                                 .str.lower()
        #                                 .map({'True': True, 'False': False, '1': True, '0': False})
        #                                 .fillna(False))
        # ana_data[c] = ana_data[c].astype(bool)

    # 控制組：兩者皆 False
    ctrl = ana_data[~ana_data[cD] & ~ana_data[cM]].copy()

    # 個案組：D 為 True；M 為 True（不互斥）
    case_D = ana_data[ana_data[cD]].copy()
    case_M = ana_data[ana_data[cM]].copy()

    # 若想互斥，改用下列兩行：
    # case_D = ana_data[ana_data[cD] & ~ana_data[cM]].copy()
    # case_M = ana_data[ana_data[cM] & ~ana_data[cD]].copy()

    comparisons = [
        ('D', 'sgl_del_sig', case_D),
        ('M', 'sgl_mis_sig', case_M),
    ]
    ...
    for mut, mut_n, case_df in comparisons:
        ...
        for abu in ['fold_change_avg']:
            ...
            print('======= compare by significance flags =======')

            # 個案組（case）與控制組（ctrl）；避免重複 transcript
            del_clash_result = case_df
            no_del_clash_result = ctrl[~ctrl['transcript_name'].isin(list(del_clash_result['transcript_name']))].copy()
            ...
            del_one_third_data   = del_clash_result[del_clash_result[score_type] <= one_third]
            ...
            for d1, d2 in zip(d1_list, d2_list):
                ...
                print(d_n, title_map_gene[str(group)], dn_list[d_n], mut_n, abu, 'with sgl_mut_stat_sig')
                # plot
                x = d1[abu]
                y = d2[abu]
                ...
                ax1.text(1.6, 0, text, fontsize=12, verticalalignment='baseline')
                ...
                text += '\n-----------------------------------------\nPermutation test: {} > {}: {}'.format(mut_n, 'com', P_m) + \
                        '\nPermutation test: {} < {}: {}'.format(mut_n, 'com', P_c)
                ax2.text(1.6, 0, text, fontsize=12, verticalalignment='baseline')

                plt.tight_layout()
                plt.savefig('figure/abu_plot/FOLD/{}_group{}_{}_{}_{}_with_sgl_mut_sig.{}'.format(
                    d_name, str(group), mut, dn_list[d_n], score_type, fig_type
                ))
                plt.clf()
                plt.close()
                gc.collect()
